...
Detailed reference for the `plimsoll estimate` command: flags, the assumptions
behind every number, the bundled node-shape catalog, storage mapping, and how to
refresh pricing. For an overview see the [README](../README.md); for how the
internals fit together see the [architecture guide](architecture.md).
